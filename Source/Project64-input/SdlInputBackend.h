...
private:
    ...
    // Raw mouse motion while captured. Reading the OS cursor runs mouse travel
    // through Windows pointer acceleration, which drops slow movement and gives
    // a dead zone. WM_INPUT reports the hardware counts unaltered, accumulated
    // here between polls; m_RawMouseSeen gates the fall back to the cursor path
    // when no raw input arrives.
    volatile LONG m_RawMouseX;
    volatile LONG m_RawMouseY;
    volatile LONG m_RawMouseWheel;
    bool m_RawMouseRegistered;
    bool m_RawMouseSeen;
};
