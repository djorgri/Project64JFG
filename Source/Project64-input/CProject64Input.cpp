...
void CProject64Input::GetKeys(int32_t Control, BUTTONS * Keys)
{
    ...
    CGuard guard(m_CS);
    if (Control == m_iFirstController || (m_GameHackMouseCapture && Control == 0))
    {
...
bool CProject64Input::GetKeyboardMouseState(KEYBOARD_MOUSE_STATE & State)
{
    CGuard guard(m_CS);
    if (m_Input.get() == nullptr)
    {
    ...
    // Game-specific keyboard/mouse mappings do not go through GetKeys(), so
    // they must refresh the raw SDL snapshot themselves. This deliberately
    // avoids evaluating the plugin's N64 button bindings.
    m_Input->UpdateDeviceData();
...
{
    CGuard guard(m_CS);
    if (Capture && m_ControlInfo.hWnd != nullptr &&
        GetForegroundWindow() != GetAncestor((HWND)m_ControlInfo.hWnd, GA_ROOT))
    {
        return;
    }
    if (m_GameHackMouseCapture == Capture)
    {
        return;
    }
    m_GameHackMouseCapture = Capture;
    if (m_Input.get() != nullptr)
    {
        m_Input->SetKeyboardMouseCapture(Capture);
    }
    PostMessage(
        (HWND)m_ControlInfo.hWnd, WM_HIDE_CUROSR,
        m_MouseLock || m_GameHackMouseCapture ? false : true, 0);
}
...
void CProject64Input::UnlockMouse()
{
    ...
    PostMessage(
        (HWND)m_ControlInfo.hWnd, WM_HIDE_CUROSR,
        m_GameHackMouseCapture ? false : true, 0);
}
