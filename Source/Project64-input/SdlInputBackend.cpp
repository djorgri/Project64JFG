...
LRESULT CALLBACK CSdlInput::PumpWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INPUT:
    {
            ...
            Raw.header.dwType == RIM_TYPEMOUSE)
        {
            ...
            {
                InterlockedExchangeAdd(&self->m_RawMouseX, Raw.data.mouse.lLastX);
                InterlockedExchangeAdd(&self->m_RawMouseY, Raw.data.mouse.lLastY);
                self->m_RawMouseSeen = true;
            }
            if ((Raw.data.mouse.usButtonFlags & RI_MOUSE_WHEEL) != 0)
            {
                InterlockedExchangeAdd(
                    &self->m_RawMouseWheel, static_cast<SHORT>(Raw.data.mouse.usButtonData));
            }
        }
        return DefWindowProcW(hwnd, msg, wParam, lParam);
    }
...
void CSdlInput::UpdateDeviceData(void)
{
    ...
    if (mit != m_Devices.end())
    {
        ...
        int wheel = 0;
        Uint32 btn = 0;
        if (m_KeyboardMouseCapture && m_hWnd != nullptr)
        {
            RECT ClientRect;
            POINT ClientOrigin = {};
            POINT Cursor;
            if (GetClientRect(m_hWnd, &ClientRect) &&
                ClientToScreen(m_hWnd, &ClientOrigin) && GetCursorPos(&Cursor))
            {
                POINT Center =
                    {
                        ClientOrigin.x + (ClientRect.right - ClientRect.left) / 2,
                        ClientOrigin.y + (ClientRect.bottom - ClientRect.top) / 2,
                    };
                if (m_RawMouseSeen)
                {
                    // Hardware counts, free of pointer acceleration. The cursor
                    // is still recentred below so it cannot drift off the window
                    // or click, but its own delta is ignored.
                    x = (int)InterlockedExchange(&m_RawMouseX, 0);
                    y = (int)InterlockedExchange(&m_RawMouseY, 0);
                }
                else
                {
                    x = Cursor.x - Center.x;
            ...
            wheel = (int)InterlockedExchange(&m_RawMouseWheel, 0);
            const int VirtualButtons[5] =
                {
                    VK_LBUTTON,
                    VK_MBUTTON,
                    VK_RBUTTON,
            ...
            {
                if ((GetAsyncKeyState(VirtualButtons[Button]) & 0x8000) != 0)
                {
                    btn |= 1u << Button;
                }
            }
        ...
        {
            btn = SDL_GetRelativeMouseState(&x, &y);
        }
        mit->second.State.Mouse.lX = (LONG)(x * 10);
        mit->second.State.Mouse.lY = (LONG)(y * 10);
        mit->second.State.Mouse.lZ = (LONG)(wheel * MOUSESCALEVALUE);
        for (int b = 0; b < 8; ++b)
        {
...
bool CSdlInput::GetKeyboardMouseState(KEYBOARD_MOUSE_STATE & State)
{
    CGuard Guard(m_DeviceCS);
    DEVICE_MAP::const_iterator Keyboard = m_Devices.find(m_GuidKeyboard);
    DEVICE_MAP::const_iterator Mouse = m_Devices.find(m_GuidMouse);
    if (Keyboard == m_Devices.end() || Mouse == m_Devices.end())
    {
        return false;
    }

    State.MouseX = Mouse->second.State.Mouse.lX / MOUSESCALEVALUE;
    State.MouseY = Mouse->second.State.Mouse.lY / MOUSESCALEVALUE;
...
void CSdlInput::SetKeyboardMouseCapture(bool Capture)
{
    CGuard Guard(m_DeviceCS);
    if (m_KeyboardMouseCapture == Capture)
    {
        return;
    }
    m_KeyboardMouseCapture = Capture;
    SDL_GetRelativeMouseState(nullptr, nullptr);
    SetRawMouseCapture(Capture);
    if (Capture && m_hWnd != nullptr)
    {
        RECT ClientRect;
        POINT ClientOrigin = {};
        if (GetClientRect(m_hWnd, &ClientRect) && ClientToScreen(m_hWnd, &ClientOrigin))
        {
            SetCursorPos(
                ClientOrigin.x + (ClientRect.right - ClientRect.left) / 2,
                ClientOrigin.y + (ClientRect.bottom - ClientRect.top) / 2);
...
// Raw mouse input is only wanted while playing, so it is registered against the
// pump window on capture and removed on release. Registering clears whatever
// accumulated so the first poll cannot inherit a stale jump, and clears the seen
// flag so the cursor path stays in use until real raw input arrives.
void CSdlInput::SetRawMouseCapture(bool Capture)
{
    if (Capture == m_RawMouseRegistered || m_PumpHwnd == nullptr)
    {
        return;
    }
    ...
    Device.dwFlags = Capture ? RIDEV_INPUTSINK : RIDEV_REMOVE;
    Device.hwndTarget = Capture ? m_PumpHwnd : nullptr;
    if (RegisterRawInputDevices(&Device, 1, sizeof(Device)) == FALSE)
    {
        return;
    }
    m_RawMouseRegistered = Capture;
    InterlockedExchange(&m_RawMouseX, 0);
    InterlockedExchange(&m_RawMouseY, 0);
    InterlockedExchange(&m_RawMouseWheel, 0);
    m_RawMouseSeen = false;
}
