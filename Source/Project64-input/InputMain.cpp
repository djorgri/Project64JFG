...
{
    if (g_InputPlugin == nullptr || State == nullptr || State->Size < sizeof(KEYBOARD_MOUSE_STATE))
    {
        return false;
    }
    return g_InputPlugin->GetKeyboardMouseState(*State) ? true : false;
}
...
EXPORT void CALL SetKeyboardMouseCapture(int32_t Capture)
{
    if (g_InputPlugin != nullptr)
    {
        g_InputPlugin->SetKeyboardMouseCapture(Capture != 0);
    }
}
...
EXPORT void CALL RomClosed(void)
{
    if (g_InputPlugin != nullptr)
    {
        g_InputPlugin->SetKeyboardMouseCapture(false);
        g_InputPlugin->UnlockMouse();
