...
void CMainGui::QueueWindowCenter(void)
{
    if (m_hMainWindow != nullptr)
    {
        PostMessage(m_hMainWindow, WM_CENTER_WINDOW, 0, 0);
    }
}

void CMainGui::CenterWindow(void)
{
    if (m_hMainWindow == nullptr || IsIconic(m_hMainWindow) || IsZoomed(m_hMainWindow) ||
        UISettingsLoadBool(UserInterface_InFullScreen))
    {
        return;
    }

    RECT windowRect;
    if (!GetWindowRect(m_hMainWindow, &windowRect))
    {
        return;
    }
    ...
    const int windowLeft = monitorInfo.rcWork.left + ((monitorInfo.rcWork.right - monitorInfo.rcWork.left) - windowWidth) / 2;
    const int windowTop = monitorInfo.rcWork.top + ((monitorInfo.rcWork.bottom - monitorInfo.rcWork.top) - windowHeight) / 2;
    SetWindowPos(m_hMainWindow, nullptr, windowLeft, windowTop, 0, 0, SWP_NOACTIVATE | SWP_NOZORDER | SWP_NOSIZE);
}
