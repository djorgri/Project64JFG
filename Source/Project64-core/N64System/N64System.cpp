...
void CN64System::GameReset()
{
    ...
    {
        m_Plugins->Control()->ResetGameHack();
    }
...
void CN64System::Reset(bool bInitReg, bool ClearMenory)
{
    ...
    // Guest-memory hooks must be removed while their original code and overlay
    // ownership tables still exist. Resetting RDRAM first would discard those
    // targets while leaving the controller runtime's bookkeeping alive.
    if (m_Plugins && m_Plugins->Control())
    {
        m_Plugins->Control()->ResetGameHack();
    }
...
void CN64System::ExecuteCPU()
{
    ...
    m_Plugins->RomOpened();
    if (m_Plugins->MainWindow() != nullptr)
    {
        m_Plugins->MainWindow()->QueueWindowCenter();
    }
    if (m_SyncCPU)
    {
        m_SyncCPU->m_Plugins->RomOpened();
...
bool CN64System::LoadState(const char * FileName)
{
    ...
    if (m_Plugins && m_Plugins->Control())
    {
        m_Plugins->Control()->GameStateLoaded();
    }
...
{
    CControl_Plugin * ControlPlugin = m_Plugins->Control();
    if (!g_SystemSettings.updateControllerOnRefresh && ControlPlugin->GetKeys != nullptr)
    {
        BUTTONS Keys;
        memset(&Keys, 0, sizeof(Keys));
        ControlPlugin->GetControllerState(Control, &Keys);
        return Keys.Value;
    }
...
void CN64System::RefreshScreen()
{
    ...
    if (g_SystemSettings.updateControllerOnRefresh && m_Plugins->Control()->GetKeys != nullptr)
    {
        ...
        {
            m_Plugins->Control()->GetControllerState(Control, &Keys);
            m_Buttons[Control] = m_SyncSystem ? g_BaseSystem->m_Buttons[Control] : Keys.Value;
        }
    }
    else
    {
        m_Plugins->Control()->UpdateGameHackInput();
    }
