...
constexpr uint32_t Start = 0x800680B0, End = 0x800681D0;
constexpr uint32_t Matrix = Start + 0x10, NumberEnter = Start + 0x50, NumberExit = Start + 0x70;
constexpr uint32_t RadarPoint = Start + 0x90;
constexpr uint32_t ReticleCapture = Start + 0xA0, ReticleSubmit = Start + 0xF8;
constexpr uint32_t ReticleTag = 0x4A46524D;
constexpr uint32_t ReticleCalls[] = {0xC68,0xCF4,0xD48,0xD9C,0xDFC,0xE4C,0xE98};
...
{
    std::vector<uint32_t> image(Original, Original + 72);
    image[0] = 0x03E00008; image[1] = 0;
    auto place = [&](uint32_t address, std::initializer_list<uint32_t> code) {
        std::copy(code.begin(), code.end(), image.begin() + (address - Start) / 4);
    };
    ...
    place(Matrix, {0x27BDFFE0, 0xAFBF0014, 0xAFA50010, Call(0x80048D84), 0,
    ...
    place(NumberEnter, {0x27BDFF40, 0xAFB30020, 0x3C19B3FF, 0xAF257FC4,
        JfgHudRaster::Jump(0x80058EF8), 0});
    place(NumberExit, {0x3C19B3FF, 0xAF207FC8, 0x03E00008, 0x27BD00C0});
    place(RadarPoint, {0x3C19B3FF, 0xAF3D7FCC, JfgHudRaster::Jump(health + 0x107C), 0});
    if (capture) {
        place(ReticleCapture, {0x27BDFFD0,0xAFA40000,0xAFA50004,0xAFA60008,
            ...
            0x8FA80020,0x15000003,0x27BD0030,0x0801B563,0,0x03E00008,0});
        place(ReticleSubmit, {0x3C08B3FF,0xAD1D7FF4,0x0801B872,0x24A53B90});
...
inline std::vector<GAME_HACK_CODE_PATCH> Hooks(uint32_t health, uint32_t multi, uint32_t reticle = 0, bool capture = true)
{
    std::vector<GAME_HACK_CODE_PATCH> hooks = {
        {0x800418D8, Call(0x80048D84), Call(Matrix)}, // camDo2DSprite (host checks caller)
        {0x80058EF0, 0x27BDFF40, JfgHudRaster::Jump(NumberEnter)},
        {0x80058EF4, 0xAFB30020, 0},
        {0x800595EC, 0x03E00008, JfgHudRaster::Jump(NumberExit)},
        {0x800595F0, 0x27BD00C0, 0},
    };
    if (health) hooks.push_back({health + 0x45C, Call(0x80048D84), Call(Matrix)});
    if (multi) for (uint32_t offset : {0x1F9Cu, 0x2204u, 0x249Cu, 0x29B0u, 0x2DB0u})
        hooks.push_back({multi + offset, Call(0x80048D84), Call(Matrix)});
    if (health && multi) hooks.push_back({multi + 0x2188, Call(health + 0x107C), Call(RadarPoint)});
    if (capture) {
        hooks.push_back({0x8006E1C0,0x3C058010,JfgHudRaster::Jump(ReticleSubmit)});
        hooks.push_back({0x8006E1C4,0x24A53B90,0x3C058010});
        if (reticle) for (unsigned i=0;i<7;++i) {
            hooks.push_back({reticle+ReticleCalls[i],0x0C01B563,Call(ReticleCapture)});
            hooks.push_back({reticle+ReticleCalls[i]+4,ReticleDelays[i],ReticleDelays[i]});
        }
    }
    return hooks;
}
inline bool Update(CGameHackMemory &memory, CGameHackCodePatcher &patcher, bool enabled)
{
    ...
    if (!owned && !matches(std::vector<uint32_t>(Original, Original + 72))) return false;
    uint8_t players = 0, mode = 0;
    uint32_t h0 = 0, h1 = 0, m0 = 0, m1 = 0;
    enabled = enabled && health && multi && memory.ReadU8(0x800A4FD0, players) &&
        players >= 2 && players <= 4 && memory.ReadU8(0x800FECA8, mode) && (mode == 1 || mode == 3) &&
        ...
        memory.ReadU32(multi + 0x43C, m0) && m0 == 0x27BDFF38 &&
        memory.ReadU32(multi + 0x440, m1) && m1 == 0xAFB50038;
    if (enabled && reticle) {
        const uint32_t signature[] = {0x27BDFF68,0xAFB4002C,0xAFB30028,0xAFB10020};
        for (unsigned i=0;i<4;++i) {
            uint32_t word=0;
            if (!memory.ReadU32(reticle+0x4A8+i*4,word) || word != signature[i]) return false;
        }
    }
