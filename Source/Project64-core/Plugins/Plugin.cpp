...
bool CPlugins::Reset(CN64System * System)
{
    ...
    if (bContChange)
    {
        // The JFG runtime owns reversible guest-memory hooks. Tear them down
        // while the live MMU is still available, before deleting the control
        // plugin and losing its original-word bookkeeping.
        if (System != nullptr && m_Control != nullptr)
        {
            m_Control->ResetGameHack();
        }
        DestroyControlPlugin();
    }
