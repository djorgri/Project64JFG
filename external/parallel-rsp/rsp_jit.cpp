...
void CPU::init_jit_thunks()
{
	...
	// GNU Lightning's x86 size estimation can fall short on MinGW32.
	// Keep room for the final branch and frame fixups because user supplied
	// code buffers cannot be resized by jit_emit().
	code_size += 4096;
...
Func CPU::jit_region(uint64_t hash, unsigned pc_word, unsigned instruction_count)
{
	...
	jit_word_t code_size;
	jit_get_code(&code_size);
	// See init_jit_thunks(): MinGW32 needs headroom for final JIT fixups.
	code_size += 4096;
